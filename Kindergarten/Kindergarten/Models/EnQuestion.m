...
-(NSString*)description
{
    ...
    if (strTip!=nil && strTip.length>0) {
        questionHtml = [NSString stringWithFormat:@"<div class=\"quescontent\">%d.%@<span style=\"color: red; font-weight: bold;display:%@\">（关键指标）</span><img src=\"images/question.png\" id=\"%@_desclink\" onclick=\"showQuesDesc(this.id)\"/><span sdesc=\"%@\" id=\"%@_desc\" /></a><div style=\"font-size:22px\" class=\"questips\"><span>%@</span></div><hr/><font style=\"display:none; font-size: 18px; border: 2px solid orange; padding: 4px;border-radius: 4px; font-weight: bold;\" color=\"orange\" id=\"quesFinish_%@\">已完成</font></div><hr/><div class=\"quescontent\" id=\"%@\">%@</div>",self.questionnum,self.content,strVeto,self.pkId,self.desc,self.pkId,strTip,self.pkId,self.pkId,optionHtml];
    }
    else
    {
        questionHtml = [NSString stringWithFormat:@"<div class=\"quescontent\">%d.%@<span style=\"color: red; font-weight: bold;display:%@\">（关键指标）</span><img src=\"images/question.png\" id=\"%@_desclink\" onclick=\"showQuesDesc(this.id)\"/><span sdesc=\"%@\" id=\"%@_desc\" /></a> <font style=\"display:none; font-size: 18px; border: 2px solid orange; padding: 4px;border-radius: 4px; font-weight: bold;\" color=\"orange\" id=\"quesFinish_%@\">已完成</font></div><hr/><div class=\"quescontent\" id=\"%@\">%@</div>",self.questionnum,self.content,strVeto,self.pkId,self.desc,self.pkId,self.pkId,self.pkId,optionHtml];
    }
