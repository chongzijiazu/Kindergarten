...
flowplayer(function(player, root) {
   ...
   player.on(FS_ENTER, function(e) {
      ...
      player.isFullscreen = false;
      win.scrollTo(scrollX, scrollY);$("#exposeMask").css("width", "100%");
   }).on('unload', function() {
...
flowplayer(function(api, root) {
   ...
   // misc
      ...
      if (error) {
         ...
         common.find('h2', el)[0].innerHTML = (api.engine && api.engine.engineName || 'html5') + ": " + conf.errors[error.code];
   ...
   bean.on(root, 'click', '.fp-fullscreen', function() { api.fullscreen(); if(BrowserSystem=="android"){if(typeof(Android)!="undefined"){Android.playFullscreen();}}});
